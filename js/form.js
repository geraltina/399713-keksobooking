...
(function () {
  window.form = (function () {
    var pinkColor = '#ff6d51';
    var greyColor = '#d9d9d3';
    ...
    accomodationPrice.addEventListener('change', function () {
      if (accomodationPrice.validity.rangeUnderflow || accomodationPrice.validity.rangeOverflow) {
        accomodationPrice.style.borderColor = pinkColor;
      } else {
        accomodationPrice.style.borderColor = greyColor;
      }
    });
    ...
    var inputsValidity = function () {
      for (var j = 0; j < inputs.length; j++) {
        if (!inputs[j].validity.valid) {
          inputs[j].style.borderColor = pinkColor;
        } else {
          inputs[j].style.borderColor = greyColor;
        }
      }
    };
    ...
    var validate = function () {
      accomodationTitle.addEventListener('change', function () {
        if (!accomodationTitle.validate.valid) {
          accomodationTitle.style.borderColor = pinkColor;
        } else {
          accomodationTitle.style.borderColor = greyColor;
        }
      });

      window.general.accomodationAddress.addEventListener('change', function () {
        if (!window.general.accomodationAddress.validity.valid) {
          window.general.accomodationAddress.style.borderColor = pinkColor;
        } else {
          window.general.accomodationAddress.style.borderColor = greyColor;
        }
      });
    };
